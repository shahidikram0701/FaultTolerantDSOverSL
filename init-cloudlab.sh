...
for vm in "${vms[@]}"; do
    ...
    ssh -o "StrictHostKeyChecking no" "$ssh_user@$vm" "git clone https://github.com/shahidikram0701/FaultTolerantDSOverSL.git && cd ~/FaultTolerantDSOverSL && git checkout feature-ds-scalog && chmod 777 init.sh && ./init.sh"
